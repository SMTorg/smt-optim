...
class MixedVarWeldedBeamDesign(BenchmarkProblem):
    ...
    def __init__(self):
        ...
        self.alt_design_space = ds.DesignSpace(
            [
                ...
                ds.FloatVariable(0.1, 10.0),  # l_param
                ds.OrdinalVariable(
    ...
    def objective(self, x):
        w, m, h, l_param, t, b = x
        w, m = int(w), int(m)
        return (1 + self.C1[m]) * (w * t + l_param) * h**2 + self.C2[m] * t * b * (
            self.L + l_param
        )
    ...
    def weld_properties(self, x):
        ...
        if w == 0:
            A = np.sqrt(2) * h * l_param
            J = np.sqrt(2) * h * l_param * ((h + t) ** 2 / 4 + l_param**2 / 12)
            R = 1 / 2 * np.sqrt(l_param**2 + (h + t) ** 2)
            cos_theta = l_param / (2 * R)
            return A, J, R, cos_theta

        # w == 1
        else:
            A = np.sqrt(2) * h * (t + l_param)
            J = np.sqrt(2) * h * l_param * (
                (h + t) ** 2 / 4 + l_param**2 / 12
            ) + np.sqrt(2) * h * t * ((h + l_param) ** 2 / 4 + t**2 / 12)
            R = max(
                1 / 2 * np.sqrt(l_param**2 + (h + t) ** 2),
                1 / 2 * np.sqrt(t**2 + (h + l_param) ** 2),
            )
            cos_theta = l_param / (2 * R) if l_param < t else t / (2 * R)
            return A, J, R, cos_theta
    ...
    def tau(self, x):
        ...
        tau_p = self.F / A
        tau_pp = (self.F * (self.L + l_param / 2) * R) / J
...
class MFWeldedBeamDesign(BenchmarkProblem):
    ...
    def expand_input(self, x, w, m, func):
        w, m = int(w), int(m)
        h, l_param, t, b = x
        x_exp = np.array([w, m, h, l_param, t, b])
        return func(x_exp)
