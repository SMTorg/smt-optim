...
class MixedVarWeldedBeamDesign(BenchmarkProblem):
    ...
    def __init__(self):
        ...
        self.design_space = ds.DesignSpace([
            ...
            ds.FloatVariable(0.0625, 2.),
            ds.FloatVariable(0.1, 10.),
            ds.FloatVariable(2., 20.),
            ds.FloatVariable(0.0625, 2.),
        ])
        ...
        self.alt_design_space = ds.DesignSpace([
            ...
            ds.OrdinalVariable(list(np.arange(0.0625, 2. + self.step, self.step))),     # h
            ds.FloatVariable(0.1, 10.),                                                # l
            ds.OrdinalVariable(list(np.arange(2., 20. + self.step, self.step))),        # t
            ds.OrdinalVariable(list(np.arange(0.0625, 2. + self.step, self.step)))      # b
        ])
    ...
    def weld_properties(self, x):
        ...
        # w == 1
            ...
            R = max(1/2*np.sqrt(l**2 + (h + t)**2), 1/2*np.sqrt(t**2 + (h + l)**2))
    ...
    def P_c(self, x):
        w, m, h, l, t, b = x
        w, m = int(w), int(m)
        return (4.013 * t * b**3 * np.sqrt(self.E[m] * self.G[m]))/(6 * self.L**2) * (1 - t/(4*self.L)*np.sqrt(self.E[m]/self.G[m]))
    ...
    def __init__(self):
        ...
        for (w, m) in self.fidelities:
            ...
            self.constraints[1].append(
                partial(self.expand_input, w=w, m=m, func=self.ref_problem.bending_stress)
            )
            ...
            self.constraints[4].append(
                partial(self.expand_input, w=w, m=m, func=self.ref_problem.side_constraints)
            )
