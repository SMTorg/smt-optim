...
class MFBorehole(BenchmarkProblem):
    ...
    def __init__(self):
        ...
        self.objective = [
            ...
            partial(self.func, A=1.),
        ]

    def set_num_fid(self, num_fidelity):

        if num_fidelity not in [2, 3]:
            warnings.warn("MF Borehole can have 2 or 3 fidelity levels. Defaulting to 2.")
            num_fidelity = 2

        if num_fidelity == 2:
            self.objective = [
                partial(self.func, A=5, B=1.5),
                partial(self.func, A=2*np.pi, B=1.),
            ]
    ...
    def func(self, x, A=2*np.pi, B=1.):

        term1 = A*x[2]*(x[3] - x[5])
        term2 = np.log10(x[1]/x[0])
        term3 = (B + (2*x[6]*x[2])/(np.log(x[1]/x[0]) * x[0]**2 * x[7]) + x[2]/x[4])
...
if __name__ == "__main__":
    ...
    x0 = np.array([7.0e-02, 2.0e+02, 7.0e+04, 1.0e+03, 1.0e+02, 7.5e+02, 1.4e+03, 1.0e+04])
    x_ref = np.array([5.00000000e-02, 2.00184597e+02, 6.99999998e+04, 9.90000000e+02,
                            9.69866639e+01, 8.20000000e+02, 1.68000000e+03, 9.96047429e+03])
